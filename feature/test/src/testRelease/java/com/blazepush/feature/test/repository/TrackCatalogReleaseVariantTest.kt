...
// + testRelease 源集组合），断言 release variant 的预置赛道列表严格为 [TFIC, XIC]，
...
/**
 * @description Release variant 预置赛道列表契约：MUST 严格为 [TFIC, XIC] 且顺序固定。
...
class TrackCatalogReleaseVariantTest {
    ...
    /**
     * Release variant 下 [PresetTrackCatalog.getAllTracks] 严格返回 [TFIC, XIC]，
     * 锁死"release 包零 debug 数据泄漏 + mainPresets 顺序固定"的契约。
     */
    @Test
    fun getAllTracks_releaseVariant_exposesTficAndXicLpccPresetsInOrder() = runTest {
        val catalog = PresetTrackCatalog()

        val ids = catalog.getAllTracks().map { it.id }

        assertEquals(listOf("preset-tfic-lpcc", "preset-xic-lpcc"), ids)
    }
}